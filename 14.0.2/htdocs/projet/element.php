...
if (strcmp($object->budget_amount, '')) {
	print price($object->budget_amount, '', $langs, 1, 0, 2, $conf->currency);
}
print '</td></tr>';

// Budget Restant
if (!empty($object->budget_amount)) {
	print '<tr><td>'.$langs->trans("BudgetRestant").'</td><td>';
	...
	print price((($object->budget_amount)-($total_commande["total_ht"])), 0, $langs, 1, 0, 2, $conf->currency);
print '</td></tr>'; }
		...
		if (is_array($elementarray) && count($elementarray) > 0) {
			...
			print '<td>';
						...
						if (!empty($object->budget_amount))
						{
							...
							print '</b>';
						/* $budget_rest = $db->query("SELECT SUM(total_ht) as total_ht FROM llx_commande_fournisseur WHERE fk_projet = " . $object->id);
						$budget_rest = $budget_rest->fetch_assoc();
						print $budget_rest["total_ht"]; */
						}	
			print '</td>';
			print '</tr>';
		} else {
