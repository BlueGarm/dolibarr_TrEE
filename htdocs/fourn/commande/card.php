...
if (empty($reshook)) {
	...
	if ($action == 'addline' && GETPOST('submitforalllines', 'aZ09') && GETPOST('vatforalllines', 'alpha') !== '' && $usercancreate) {
		...
		if ($prod_entry_mode != 'free' && empty($error)) {	// With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
			...
			if ($idprod > 0) {
				...
				// Modification : Utilisation de la référence founisseurs saisie ou système
	...
	if ($action == 'add' && $usercancreate) {
		...
		if (!$error) {
			...
			$object->date_livraison = $datelivraison; // Modification
...
if ($action == 'create') {
	...
	if (!empty($origin) && !empty($originid)) {
		...
		$datedelivery = (!empty($objectsrc->date_livraison) ? $objectsrc->date_livraison : ''); // Modification
	...
	if (isModEnabled('project')) {
		...
		if ($permissiontoadd) {
			...
			// Modification : Affichage Budget restant
			if ($action == 'classify') {
				...
				if (!empty($object->fk_project)) {
					// Récupération somme des commandes sur projet
					$total_commande = $db->query("SELECT SUM(total_ht) as total_ht FROM llx_commande_fournisseur WHERE fk_projet = " . $object->fk_project);
					$total_commande = $total_commande->fetch_assoc();
					// Récupération budget projet
					$budget_a = $db->query("SELECT budget_amount as budget_amount FROM llx_projet WHERE rowid = " . $object->fk_project);
					$budget_a = $budget_a->fetch_assoc();
					// Calcul budget restant
					$budget_rest = (($budget_a["budget_amount"])-($total_commande["total_ht"]));
					// Mise à jour du budget restant
					$update_budget_rest = $db->query("UPDATE llx_projet SET budget_rest = " . $budget_rest . " WHERE llx_projet.rowid = " . $object->fk_project);
					// Formattage budget restabt
					$budget_rest = price($budget_rest, '', $langs, 1, 0, 2, $conf->currency);
					// Paramètres en fonction du solde restant
					if ($budget_rest < 0 ) {
						$negative_balance = ' negative_balance'; 
						$picto = img_picto($langs->trans("NegativeBalance"), "warning").' ';
					}
					// Affichage sous projet si budget indiqué
					if (!empty($budget_a["budget_amount"])) {
						$morehtmlref .= '<span class="budget_info budget_amount'.$negative_balance.'">'.$picto.$langs->trans("BudgetBalance").' : '.$budget_rest.'</span>';
					}
					// Accord Porteur
					$orderconfirmation = $db->query("SELECT order_confirmation as order_confirmation FROM llx_projet_extrafields WHERE fk_object = " . $object->fk_project);
					$orderconfirmation = $orderconfirmation->fetch_assoc();
					if ($orderconfirmation["order_confirmation"] == 1) {
						$picto = img_picto($langs->trans("Attention"), "warning").' ';
						$morehtmlref .= '<span class="budget_info budget_confirmation">'.$picto.$langs->trans("ValidationRequiredByOwner").'</span>';;
					}
				}
			}
		} else {
	...
	if (empty($reshook)) {
		...
		if ($object->methode_commande_id > 0) {
			...
			if ($object->hasDelay() && empty($object->delivery_date) && !empty($object->date_commande)) { // Modification : Alerte Retard date de commande/Date de livraison fiche commande
