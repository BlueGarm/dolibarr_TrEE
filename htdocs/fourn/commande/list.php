...
$sql .= " cf.rowid, cf.ref, cf.ref_supplier, cf.fk_statut, cf.billed, cf.total_ht, cf.total_tva, cf.total_ttc, cf.fk_user_author, cf.date_commande as date_commande, cf.date_livraison as date_livraison, cf.date_valid, cf.date_approve,"; // Modification
...
if ($resql) {
	...
	while ($i < $imaxinloop) {
		...
		$objectstatic->delivery_date = $db->jdate($obj->date_livraison); // Modification
...
// Order date // Modification : Alerte Retard date de commande/Date de livraison liste commande
			if (!empty($arrayfields['cf.date_commande']['checked'])) {
				print '<td class="center">';
			...
			if (!empty($arrayfields['cf.date_livraison']['checked'])) {
				...
				if ($objectstatic->statut == $objectstatic::STATUS_ORDERSENT || $objectstatic->statut == $objectstatic::STATUS_RECEIVED_PARTIALLY) {
					if ($objectstatic->hasDelay() && !empty($objectstatic->delivery_date)) { // Modification : Alerte date de livraison
						print ' '.img_picto($langs->trans("Late").' : '.$objectstatic->showDelay(), "warning");
					}
				}
				print '</td>';
