...
/**
 *	\file       htdocs/core/modules/supplier_order/doc/pdf_TrEE.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from TrEE model
 */
...
/**
 *	Class to generate the supplier orders with the TrEE model
 */
class pdf_TrEE extends ModelePDFSuppliersOrders
{
	/**
	...
	public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->fournisseur->commande->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$heightforinfotot = 25; // Height reserved to output the info and total part // Modification : Espace Totaux
				...
				// Modification : Extrafields Document
				...
				// Modification : Titre Note et Information
				$pdf->SetFont('', '', $default_font_size - 2);
				$pdf->SetFillColor(240, 240, 240);
				$pdf->writeHTMLCell (190,3, $this->posxdesc-1, $tab_top-6.5,'<b>Note & Informations</b>',0,1,1);
				...
				for ($i = 0; $i < $nblines; $i++) {
					$object->lines[$i]->fetch_optionals($object->lines[$i]->rowid,$extralabelsline); // Modification : Extralabels ligne
					// Modification : Extralabels depuis produit-service
					if ($object->lines[$i]->fk_product)	{
						...
						unset ($product, $extrafields_product, $extralabels_product ) ; // Modification : Evite les doublons
					}
					...
					$pdf->setTopMargin($tab_top_newpage);
					$pdf->setPageOrientation('', 1, $heightforfooter + $heightforfreetext); // The only function to edit the bottom margin of current page to set it. // Modification : Pas de zone vide sur page sans zone total
					...
					if ($this->getColumnStatus('desc')) {
						...
						// Modification : Custom ExtraFields
						// NACRE v3
						if (!empty ($product->array_options['options_nacre'])) {
							$extra_nacre=$extrafields_product->showOutputField('nacre', $product->array_options['options_nacre'],'',$product->table_element);
						} elseif (!empty ($object->lines[$i]->array_options['options_cnacre'])) {
							$extra_nacre=$extrafieldsline->showOutputField('cnacre', $object->lines[$i]->array_options['options_cnacre'],'',$object->table_element_line);
						} else {
							$extra_nacre='';
						}
						...
						$pdf->SetFont('', '', $default_font_size - 2);
						$pdf->SetFillColor(240, 240, 240);
						$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, ' • <i><b>Information(s) Produit</b></i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Code NACRE : </b>'.$extra_nacre.'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						
						// Produit Pour
						if (!empty ($object->lines[$i]->array_options['options_product_for'])) {
							$extra_product_for=$extrafieldsline->showOutputField('product_for', $object->lines[$i]->array_options['options_product_for'],'',$object->table_element_line);
							$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Produit pour : </b>'.$extra_product_for.'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						}

						// Devis
						if (!empty ($object->lines[$i]->array_options['options_offerproduct'])) {
							$extra_offerproduct=$outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_offerproduct']);
							$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Devis/Offre : </b>'.$extra_offerproduct.'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						}
