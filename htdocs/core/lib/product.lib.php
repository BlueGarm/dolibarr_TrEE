...
function product_prepare_head($object)
{
	...
	if (!empty($object->status)) { //Modification : Réactivation onglet masqué si non en vente
	...
	if (!empty($object->status_buy) || (isModEnabled('margin') && !empty($object->status))) {   // Modification : Réactivation onglet masqué si non en achat // If margin is on and product on sell, we may need the cost price even if product os not on purchase
