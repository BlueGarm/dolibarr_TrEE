	...
	foreach ($extrafields->attributes[$object->table_element]['label'] as $tmpkeyextra => $tmplabelextra) {
		...
		if ($extrafields->attributes[$object->table_element]['type'][$tmpkeyextra] == 'separate') {
			...
			if ($action == 'edit_extras' && $permok && GETPOST('attribute', 'restricthtml') == $tmpkeyextra) {
				...
				// Modification : Facilitation Mise en Forme
				$extrafieldsToShow = $extrafields->showOutputField($tmpkeyextra, $value, '', $object->table_element);
				$title = dol_string_nohtmltag($extrafieldsToShow);
				print($title ? '<div title="'.dol_escape_htmltag($title).'">'.$extrafieldsToShow.'</div>' : $extrafieldsToShow);
			}
