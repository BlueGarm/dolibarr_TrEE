...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	<td class="nobottom linecoldescription minwidth400imp">
		...
		$doleditor = new DolEditor('dp_desc', GETPOST('dp_desc', 'restricthtml'), '', (!getDolGlobalString('MAIN_DOLEDITOR_HEIGHT') ? 100 : $conf->global->MAIN_DOLEDITOR_HEIGHT), $toolbarname, '', false, true, /*$enabled*/ 0 /* Modification : Désactive CKE pour commande*/ , $nbrows, '98%');
		$doleditor->Create();
...
?>
	...
	function setforfree() {
		...
		jQuery("#price_ht, #multicurrency_price_ht, #price_ttc, #multicurrency_price_ttc, #fourn_ref, #tva_tx, #buying_price, #title_fourn_ref, #title_vat, #title_up_ht, #title_up_ht_currency, #title_up_ttc, #title_up_ttc_currency, .commande_fournisseurdet_extras_cnacre, .commande_fournisseurdet_extras_add_bdd").show(); // Modification : Affichage extralabels
	...
	function setforpredef() {
		...
		<?php } ?>
		/*jQuery("#fourn_ref, #tva_tx, #title_vat").hide(); // Modification : Réference Fournisseur
		jQuery("#title_fourn_ref").hide();
		jQuery("#np_marginRate, #np_markRate, .np_marginRate, .np_markRate, #units, #title_units").hide();*/
		jQuery("#buying_price, #fourn_ref").show(); // Modification : Affichage Champs Référence fournisseurs même sur produit prédéfinie
		jQuery('#trlinefordates, .divlinefordates').show();
		jQuery(".commande_fournisseurdet_extras_cnacre.trextrafields_collapseorder_info_sup, .commande_fournisseurdet_extras_add_bdd.trextrafields_collapseorder_info_sup").hide(); // Modification : Masquage code NACRE et Ajout BDD sur produit prédéfinie
	}
