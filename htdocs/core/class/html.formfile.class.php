	...
	public function getDocumentsLink($modulepart, $modulesubdir, $filedir, $filter = '', $morecss = 'valignmiddle', $allfiles = 0)
	{
		...
		if (!empty($file_list)) {
			$out = '<dl class="dropdown inline-block">
    			...
    			<dd><div class="multichoicedoc" style="position:absolute;left:100px;" ><ul class="ulselectedfields" style="position:absolute; right: unset!important; left: -100px!important;">'; // Modification : Affichage correct ul
			$tmpout = '';
			...
			foreach ($file_list as $file) {
				...
				if (!empty($conf->use_javascript_ajax) && ($conf->browser->layout != 'phone')) {
					...
					if ($tmparray && $tmparray['url']) {
						...
						$tmpout .= $langs->trans("Preview").' '.$ext.'</a><span style="color:gray;font-style:italic;">(' .$file["name"].')</span></li>'; // Modification : Affichage du nom du ficher
					}
				}
				...
				$tmpout .= '</a><span style="color:gray;font-style:italic;">(' .$file["name"].')</span></li>'."\n";  // Modification : Affichage du nom du ficher
			}
			$out .= $tmpout;
			$out .= '</ul></div></dd>
    			</dl>';
