...
class ExtraFields
{
		...
		if ($type == 'date') {
			...
			if (is_array($value_arr)) {
				foreach ($value_arr as $keyval => $valueval) {
					if (!empty($valueval)) {
						$toprint[] = '<li title="'.strip_tags($param['options'][$valueval]).'" class="select2-search-choice-dolibarr noborderoncategories" style="background: #bbb">'.$param['options'][$valueval].'</li>'; // Modification : Ajout titre
					}
				}
			}
			$value = '<div class="select2-container-multi-dolibarr" style="width: 90%;"><ul class="select2-choices-dolibarr">'.implode(' ', $toprint).'</ul></div>';
	...
	public function showSeparator($key, $object, $colspan = 2, $display_type = 'card', $mode = '')
	{
		...
		// Output the picto
		$out .= '<span class="'.($extrafield_collapse_display_value ? 'cursorpointer ' : '').($extrafield_collapse_display_value == 0 ? 'fas fa-angle-right opacitymedium' : 'far fa-'.(($expand_display ? 'minus' : 'plus').'-square')).'"></span>'; // Modification : Remplacement puce 
		$out .= '&nbsp;';
		$out .= '<strong>';
