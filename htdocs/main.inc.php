...
if (getDolGlobalString('MAIN_ONLY_LOGIN_ALLOWED')) {
	...
	if (!$ok) {
		if (session_id() && isset($_SESSION["dol_login"]) && $_SESSION["dol_login"] != $conf->global->MAIN_ONLY_LOGIN_ALLOWED) {
			// Modification : Traduction Mode Maintenance
			print 'Désolé, votre application est hors ligne pour maintenance.'."\n";
			print 'Vous êtes connecté en tant que "'.$_SESSION["dol_login"].'" and only administrator user "' . getDolGlobalString('MAIN_ONLY_LOGIN_ALLOWED').'" et seul l\'administrateur "'.$conf->global->MAIN_ONLY_LOGIN_ALLOWED.'" est autorisé à se connecter pour le moment.'."\n";
			$nexturl = DOL_URL_ROOT.'/user/logout.php?token='.newToken();
			print 'Veuillez réessayer plus tard ou <a href="'.$nexturl.'">cliquez ici pour vous déconnecter et changer d\'utilisateur</a>...'."\n";
		} else {
			print 'Désolé, votre application est hors ligne pour maintenance. Seul l\'administrateur "' . getDolGlobalString('MAIN_ONLY_LOGIN_ALLOWED').'" est autorisé à se connecter pour le moment.'."\n";
