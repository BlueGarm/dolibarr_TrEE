...
/**
 *	\file       htdocs/core/modules/supplier_order/pdf/pdf_Margaux.modules.php
 *	\ingroup    fournisseur
 *	\brief      File of class to generate suppliers orders from Margaux model
 */
...
/**
 *	Class to generate the supplier orders with the Margaux model
 */
class pdf_Margaux extends ModelePDFSuppliersOrders
{
    /**
	...
	public function __construct($db)
	{
		...
		{
			$this->posxedp=$this->marge_gauche+1;
			$this->posxtva=125;
			$this->posxup=125;
			$this->posxqty=145;
			$this->posxunit=155;
		...
		// Extralabels depuis produit-service
			if ($object->lines[$i]->fk_product)
			{
				...
				// Extra NACRE
						...
						//$pdf->MultiCell(115, 3, '<i><b>Code NACRE : </b>'.$extrafields_product->showOutputField('nacre', $product->array_options['options_nacre']).'</i>',0 , 'L', 0, 1, '', '', true, 0, true, true, 0);

				// Extra NACRE v2 - Masqué si vide
				if (empty ($product->array_options['options_nacre']))
						{
							$pdf->SetFont('', 'B', $default_font_size - 2);
							$nacreh = $product->array_options['options_nacre']; // je recupere la valeur et je la masque
						}
						else
						{
							$pdf->SetFont('', '', $default_font_size - 2);
							$pdf->SetFillColor(240, 240, 240);
							$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Code NACRE : </b>'.$extrafields_product->showOutputField('nacre', $product->array_options['options_nacre']).'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						}
				// Custom NACRE - Masqué si vide
					if (empty ($object->lines[$i]->array_options['options_cnacre']))
						{
							$pdf->SetFont('', 'B', $default_font_size - 2);
							$nacre_custom = $object->lines[$i]->array_options['options_cnacre']; // je recupere la valeur
						}
						else
						{
							$pdf->SetFont('', '', $default_font_size - 2);
							$pdf->SetFillColor(240, 240, 240);
							$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Code NACRE : </b>'.$extrafields_line->showOutputField('cnacre', $object->lines[$i]->array_options['options_cnacre']).'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						}
				
				// EDP
					// if (empty ($object->lines[$i]->array_options['options_budg']))
						// {
							// $pdf->SetFont('', 'B', $default_font_size - 2);
							// $budg_custom = $object->lines[$i]->array_options['options_budg']; // je recupere la valeur // je l'affiche dans le pdf
						// }
						// else
						// {
							// $pdf->SetFont('', '', $default_font_size - 2);
							// $pdf->SetFillColor(240, 240, 240);
							// $budg_custom = $object->lines[$i]->array_options['options_budg']; // je recupere la valeur
							// $pdf->MultiCell(115, 3, $outputlangs->convToOutputCharset('<b>EDP : </b>'.$budg_custom), 0, 'L', 1, 1, '', '', true, 0, true, true, 0); // je l'affiche dans le pdf
						// }

				// EDP v2 - Masqué si vide
					if (empty ($object->lines[$i]->array_options['options_projedp']))
						{
							$pdf->SetFont('', 'B', $default_font_size - 2);
							$budg_custom = $object->lines[$i]->array_options['options_projedp']; // je recupere la valeur
						}
						else
						{
							$pdf->SetFont('', '', $default_font_size - 2);
							$pdf->SetFillColor(240, 240, 240);
							$pdf->MultiCell($this->posxup-$this->posxdesc+1, 3, '<i><b>Autre ligne : </b>'.$outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_projedp']).'</i>',0 , 'L', 1, 1, '', '', true, 0, true, true, 0);
						}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("TotalHT"), 0, 'C', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (! empty($conf->global->PDF_SHOW_PROJECT))
		{
			...
			if (! empty($object->project->ref))
			{
				...
				$langs->load("projects");
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell(100, 3, '<b>'.$outputlangs->transnoentities("Project")." : " .'</b>'. (empty($object->project->ref)?'':$object->projet->ref), '', 'R', 0, 1, '', '', true, 0, true, true, 0);
			}
		}
