...
class box_supplier_orders_awaiting_reception extends ModeleBoxes
{
    ...
    public function loadBox($max = 5)
    {
        ...
        if ($user->rights->fournisseur->commande->lire)
        {
            ...
            $sql .= " s.logo,";
            $sql .= " c.rowid, c.ref, c.tms, c.date_commande, c.date_livraison as date_delivery, ";
            $sql .= " c.total_ht,";
            ...
            $sql .= " AND fk_statut IN (".CommandeFournisseur::STATUS_ORDERSENT.",".CommandeFournisseur::STATUS_RECEIVED_PARTIALLY.")";
            ...
            if ($result)
            {
                ...
                while ($line < $num) {
                    ...
                    $date_livraison = $this->db->jdate($objp->date_delivery);
					$datem = $this->db->jdate($objp->tms);

                    $supplierorderstatic->date_delivery = $date_livraison;
		    $supplierorderstatic->date_commande = $date;
                    ...
                    if (!empty($date_livraison)) {
						$this->info_box_contents[$line][] = array(
													'td' => 'class="right"',
							'text' => $delayIcon.'<span class="classfortooltip" title="'.$langs->trans('DateDeliveryPlanned').'"><i class="fa fa-dolly" style="margin-left: 5px;"></i> '.dol_print_date($date_livraison, 'day').'</span>',
							'asis' => 1
						);
					}
					else {
