...
class CommandeFournisseur extends CommonOrder
{
    ...
    public function load_board($user, $mode = 'opened')
    {
        ...
        $sql = "SELECT c.rowid, c.date_creation as datec, c.date_commande, c.fk_statut, c.date_livraison as date_delivery";
        $sql .= " FROM ".MAIN_DB_PREFIX."commande_fournisseur as c";
        if (!$user->rights->societe->client->voir && !$user->socid)
        {
        ...
        if ($mode === 'awaiting') {
            $sql .= " AND fk_statut IN (".self::STATUS_ORDERSENT.",".self::STATUS_RECEIVED_PARTIALLY.")";
        }
        else {
        ...
        if ($resql)
        {
            ...
            while ($obj = $this->db->fetch_object($resql))
            {
                ...
                $commandestatic->date_livraison = $this->db->jdate($obj->date_delivery);
				$commandestatic->date_delivery = $this->db->jdate($obj->date_delivery);
