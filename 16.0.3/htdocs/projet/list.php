...
// Modification : Budget Retant
$search_budget_rest = GETPOST("search_budget_rest", 'alpha');
$search_budget_percent = GETPOST("search_budget_percent", 'alpha');
...
if (empty($reshook)) {
	...
	if (GETPOST('button_removefilter_x', 'alpha') || GETPOST('button_removefilter.x', 'alpha') || GETPOST('button_removefilter', 'alpha')) { // All tests are required to be compatible with all browsers
		...
		// Modification : Budget Restant
		$search_budget_rest = '';
		$search_budget_percent = '';
		$search_public = "";
		$search_sale = "";
		$search_project_user = '';
...
$sql .= " p.datec as date_creation, p.dateo as date_start, p.datee as date_end, p.opp_amount, p.opp_percent, (p.opp_amount*p.opp_percent/100) as opp_weighted_amount, p.tms as date_update, p.budget_amount,p.budget_rest,p.budget_percent,"; // modification : Budget Restant
...
// Modification : Budget Restant [539-545]
if ($search_budget_rest != '') {
	$sql .= natural_search('p.budget_rest', $search_budget_rest, 1);
}
if ($search_budget_percent != '') {
	$sql .= natural_search('p.budget_percent', $search_budget_percent, 1);
}
...
// Modification : Budget Restant [740-746]
if ($search_budget_rest != '') {
	$param .= '&search_budget_rest='.urlencode($search_budget_rest);
}
if ($search_budget_percent != '') {
	$param .= '&search_budget_percent='.urlencode($search_budget_percent);
}
...
if (!empty($arrayfields['p.budget_amount']['checked'])) {
	print '<td class="liste_titre nowrap right">';
	// Modification : Budget Restant [9951006]
	//print '<input type="text" class="flat" name="search_budget_amount" size="4" value="'.$search_budget_amount.'">';
	print '</td>';
}
if (!empty($arrayfields['p.budget_rest']['checked'])) {
	print '<td class="liste_titre nowrap right">';
	//print '<input type="text" class="flat" name="search_budget_rest" size="4" value="'.$search_budget_rest.'">';
	print '</td>';
}
if (!empty($arrayfields['p.budget_amount']['checked'])) {
	print '<td class="liste_titre nowrap right">';
	// print '<input type="text" class="flat" name="search_budget_percent" size="4" value="'.$search_budget_percent.'">';
	print '</td>';
}
...
// Modification Budget Restant [1131-1137]
if (!empty($arrayfields['p.budget_rest']['checked'])) {
	print_liste_field_titre($arrayfields['p.budget_rest']['label'], $_SERVER["PHP_SELF"], 'p.budget_rest', "", $param, '', $sortfield, $sortorder, 'right ');
}
if (!empty($arrayfields['p.budget_amount']['checked'])) {
	print_liste_field_titre($arrayfields['p.budget_percent']['label'], $_SERVER["PHP_SELF"], 'p.budget_percent', "", $param, '', $sortfield, $sortorder, 'right ');
}
if (!empty($arrayfields['p.usage_opportunity']['checked'])) {
	...
	if ($userAccess >= 0) {
		...
		if (!empty($arrayfields['p.budget_amount']['checked'])) {
			...
			if ($obj->budget_amount != '') {
				print '<span class="amount">'.price($obj->budget_amount, '', $langs, 1, 0, 2, $conf->currency).'</span>'; // Modification : Affichage Correct Prix
		...
		// Modification : Budget_restant [1439-1481]
		if (!empty($arrayfields['p.budget_rest']['checked']))  {
			print '<td class="right">';
			if ($obj->budget_amount != '') {
				// Mise a Jour de la BDD
				$total_commande = $db->query("SELECT SUM(total_ht) as total_ht FROM llx_commande_fournisseur WHERE fk_projet = " . $object->id);
		...
		if (!empty($arrayfields['p.budget_amount']['checked'])) {
			...
			if ($obj->budget_amount != 0 ) {
				...
				$total_commande = $db->query("SELECT SUM(total_ht) as total_ht FROM llx_commande_fournisseur WHERE fk_projet = " . $object->id);
				$total_commande = $total_commande->fetch_assoc();
				$budget_percent = ((($total_commande["total_ht"])*100)/($obj->budget_amount));
				$update_budget_percent = $db->query("UPDATE llx_projet SET budget_percent = " . $budget_percent . " WHERE llx_projet.rowid = " . $object->id);
				// Affichage Pourcentage Correct
				print '<span class="amount';
				if ($budget_percent > 100) {
					print ' more100percent';
				}
				print '">'.number_format($budget_percent, 2, ',', ' ').'%</span>';
			}
			print '</td>';
		}
		// Usage opportunity
