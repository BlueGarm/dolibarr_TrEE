	...
	if ($action == 'addline' && $usercancreate) {
		...
		if ($prod_entry_mode != 'free' && empty($error)) {	// With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
			...
			if ($idprod > 0) {
				...
				// Modification : Utilisation de la référence founisseurs saisie ou système [588-601]
...
if ($action == 'create') {
	...
	if (!empty($conf->project->enabled)) {
		...
		if ($usercancreate) {
			...
			if ($action == 'classify') {
				...
				// Modification : Affichage Budget restant [2069-2093]
				if (!empty($object->fk_project)) {
					...
					if ($budget_rest < 0 ) {
						$color = '#bb0b0b'; 
						$picto = img_picto($langs->trans("Solde Négatif"), "warning");
						$weight = 'bold';
					}
					// Affichage sous projet si budget indiqué
					if (!empty($budget_a["budget_amount"])) {
						$morehtmlref .= '<span style="font-weight: '.$weight.'; color: '.$color.'; display: block;">'.$picto.' Budget restant : '.$budget_rest.'</span>';
					}
				}
			}
		} else {
	...
	if ($object->methode_commande_id > 0) {
		...
		if ($object->hasDelay() && empty($object->date_delivery) && !empty($object->date_commande)) { // Modification : Alerte Retard date de commande/Date de livraison fiche commande
